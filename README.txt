...
3. Start the game and and in the chat type "/grant singleplayer creative" (There's a privilege that needs to be added so the jetpack doesn't die once it is used up).
4. In the chat type "/level 1"
5. With the jetpack in your hand (and looking up not at the ground), hold down the right mouse button until you hear the jetpack start.
6. Play around with the spacebar and the WASD keys until you get used to the flight mechanic.
7. When you hear the jetpack chirp 3 times that means the jetpack is running out of fuel, so throttle the spacebar on and off for a soft landing.
8. When the jetpack runs out of fuel and stops, look up and hold the right mousebutton down again to restart the jetpack.
9. To turn off the jetpack press shift.
...
13. The goal eventually will be to find the trapped miner at the bottom of the mine.  For now just find the chest at the bottom of the mine.
14. Press the space bar to keep from falling into the mine and injuring yourself
15. If a cobble wall is blocking your path, you can blast a hole in it using your Hero TNT and lighting it with a flint.
